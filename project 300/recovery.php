...
if(isset($_POST['submit']))
{
    ...
    if($new_password == $retype_password)
    {
        ...
        $update_password= $GLOBALS['collection']->updateOne(array('_id' => new MongoDB\BSON\ObjectID( $userID)),array('$set'=>array('password' => $new_password )));
        if($update_password)
        {
                // mysqli_query($db, "UPDATE recovery_keys SET valid = 0 WHERE userID = $userID AND token ='$token'");
            $GLOBALS['collectionRecovery']->updateOne(array('token' => $token),array('$set'=>array('valid' => 0 )));
            
                 $msg = 'Your password has changed successfully. Please login with your new passowrd.';
                $msgclass = 'bg-success';
                echo '<script type="text/javascript">alert("Your password has changed successfully. Please login with your new passowrd.");
     </script>';
...
<nav class="navbar navbar-default">
  <div class="container-fluid"> 
    ...
    <div class="navbar-header">
      ...
      <a class="logo" href="#"><img src="image/logo.png" width=45px height=40px  /></a></div>

  ...
  <div class="modal-dialog modal-sm">
    <div class="modal-content">
      <div class="modal-header">
        ...
        <h4 class="modal-title" id="myModalLabel">confirmation msg :</h4>
      </div>
      ...
      <div class="modal-footer">
        ...
        <input type="hidden" name="delreportId" value="<?php echo $reportData['_id']; ?>">
         <input type="hidden" name="deluserId" value="<?php echo $result['_id']; ?>">
        <input type="submit" value="Confirm" name="delinfo" class="btn btn-success" />
        </form>
      </div>
    </div>
  </div>
...
<?php
       if(isset($_POST['delinfo']))
       {
        ...
        $delreport=$_POST['delreportId'];
        // $collectionReport->deleteOne(array('_id' => new MongoDB\BSON\ObjectID($delreport)));
        // $collectionReport->remove($delreport);
         $result = $collection->updateOne(
     array('_id' => new MongoDB\BSON\ObjectID( $deluser)),
     array(
          '$pull'=> array('reports' => array('$id' => new MongoDB\BSON\ObjectID( $delreport)))
          )
 );
$collectionReport->deleteOne(array('_id' => new MongoDB\BSON\ObjectID($delreport)));
         // redirect("UserProfile.php");
      $str='Location:UserProfile.php?source='.$id1;
      header($str);
